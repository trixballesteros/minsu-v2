            ...
            <div class="content">
                <div class="row align-items-center py-4">
                    <div class="col-lg-6 col-7">
                        <h6 class="h2 d-inline-block mb-0">Downloadables</h6>
                        <nav
                ...
                <form @submit.prevent="submit">
                    <modal v-model:show="modals.downloadables">
                        <div class="text-left">
                            <base-input
                                label="Name"
                                name="name"
                        ...
                        <template v-slot:footer>
                            <div v-if="!modals.isViewDetails">
                                <base-button
                                    type="secondary"
                                    @click="modals.downloadables = false"
                                >Close
                                </base-button
                                >
...
export default {
    ...
    computed: {
        ...mapGetters("DownloadablesIndex", [
            "tableData",
            "pagination",
            "tableColumns",
            "serverQuery",
        ]),
        ...mapGetters("DownloadablesSingle", ["item", "formErrors"]),
    },
    unmounted: function () {
        this.resetDownloadablesState();
        this.resetState();
    },
    methods: {
        ...mapActions("DownloadablesIndex", [
            "fetchData",
            "setServerQuery",
            "destroyData",
        ]),
        ...mapActions("DownloadablesIndex", {resetDownloadablesState: "resetState"}),
        onFileChange(event) {
            // const files = event.target.files || event;
            // // If the file is valid, dispatch the Vuex action to store the file
            // if (files && files.length > 0) {
            //     this.setDownloadablesFile(files[0]);
            // }
            const files = event?.target?.files || event;
            if (files && files.length > 0) {
                this.setDownloadablesFile(files[0]);
        ...
        ...mapActions("DownloadablesSingle", [
            "setName",
            "setDownloadablesFile",
            "setParentId",
            "setItem",
            "storeData",
            "updateData",
        ...
        viewDetails(selectedItem) {
            ...
            this.modals.downloadables = true;
            this.setItem({...selectedItem});
        },
        closeModal() {
            this.modals.downloadables = false;
        },
