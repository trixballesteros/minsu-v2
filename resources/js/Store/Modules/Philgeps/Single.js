...
const actions = {
    storeData({ commit, state, dispatch }) {
        ...
        return new Promise((resolve, reject) => {
            axios.post(route('api.philgeps.store'), params)
                .then(response => {
    ...
    updateData({ commit, state, dispatch }) {
        ...
        return new Promise((resolve, reject) => {
            axios.post(route('api.philgeps.update', state.item.id),params)
                .then(response => {
    ...
    setPhilgepsFile({commit}, value)
    {
        commit('setPhilgepsFile', value)
    },
...
const mutations = {
    ...
    setPhilgepsFile(state, value){
        state.item.philgeps_file =  value && value.length > 0 ? value[0] : value
        state.formErrors.philgeps_file = null
    },
